---
title: "Problems of unbalancedness with applications in R"
...
Each type of the unbalancedness can be considered as **missing data**. [reference](https://link.springer.com/chapter/10.1007/978-3-030-53953-5_9)
...
In this section, The practical examples are built around Grunfeld’s Investment Data contained in R package plm. This dataset is a panel of 10 observational units (firms) from 1935 to 1954. [reference](https://search.r-project.org/CRAN/refmans/plm/html/punbalancedness.html)
...
#### 2.6 The `na.action` Argument in `lm()` and `plm()`

If the data contains missing values, `lm()` automatically deletes the whole observation. In `lm()` function you can deal with **missing values** with setting `na.action` in four ways: 

* `na.action = na.fail`: Getting an error when the data contains missing values.
* `na.action = na.omit`: Removing an entire row if it contains missing values. With `na.omit` you will end up with fewer rows. 
* `na.action = na.exclude`: Removing an entire row if it contains missing values. It retains the original number of rows in the data. It is useful when you want to compare predicted values to original values.
* `na.action = na.pass`: Passing with missing values. 

### 3. Dip dive in handling missing data

Some useful references: [link1](https://rpubs.com/odenipinedo/dealing-with-missing-data-in-R), [link2](https://www.r-bloggers.com/2021/04/handling-missing-values-in-r/), [link3](https://medium.com/coinmonks/dealing-with-missing-data-using-r-3ae428da2d17), [link4](https://epirhandbook.com/en/missing-data.html)

#### 3.1 Create MCAR and MAR variables to the dataset

Definitions of missingness process are tricky. In the following we have the definitions: 

1. **Missing at random (MAR)**: This is confusing and would be better named missing conditionally at random. Here, missingness in a particular variable has an association with one or more other variables in the dataset. However, the actual values of the missing data are random. 

2. **Missing completely at random (MCAR)**: As it says, values are randomly missing from your dataset. Missing data values do not relate to any other data in the dataset and there is no pattern to the actual values of the missing data themselves. In our example, smoking status is missing from a random subset of male and female patients. 

3. **Missing not at random (MNAR)**: The pattern of missingness is related to other variables in the dataset, but in addition, the actual values of the missing data are not random. In our example, smoking status is missing in female patients who are more likely to smoke, but not for male patients. 
...
In deciding whether data is MCAR or MAR, one approach is to explore patterns of missingness between levels of included variables. This is particularly important for a primary outcome measure/dependent variable. [reference1](https://cran.r-project.org/web/packages/finalfit/vignettes/missing.html), [reference2](https://argoshare.is.ed.ac.uk/healthyr_book/handling-missing-data-mar.html)
...
Before this part, we studied how to decide about type of our missing data category. Based on that, we choose a method for handling missing data. Prior to a standard analysis, we can either [reference](http://rstudio-pubs-static.s3.amazonaws.com/384609_4e5f7518031b492ebd949eb895a8b416.html):
...
**Common solution: row-wise deletion**: Depending on the number of data points that are missing, we may have sufficient power with complete cases to examine the relationships of interest.

We therefore select to omit the patients in whom smoking is missing. This is known as list-wise deletion and will be performed by default and usually silently by any standard regression function.
...
To showing effectiveness of the ways, in this part we use another dataset. But do not get confused! We can easily apply them to any dataset that we want. [reference1](http://juliejosse.com/wp-content/uploads/2018/06/DataAnalysisMissingR.html), 
[reference2](https://github.com/dalpozz/unbalanced)
...
**Explaination of the method**: The basic idea behind the algorithm is to treat each variable that has missing values as a dependent variable in regression and treat the others as independent (predictors). ([More about this algorithm in details](https://www.rdocumentation.org/packages/mice/versions/3.16.0/topics/mice))
...
For now, I use `new_Data` from part `2.4` which is an unbalanced dataset with `lm()` Function [reference](https://cran.r-project.org/web/packages/plm/vignettes/A_plmPackage.html):
...
In this section, we create a new simple dataset to show to the analysis [reference](https://www.annualreviews.org/doi/pdf/10.1146/annurev-publhealth-040617-013507): 
